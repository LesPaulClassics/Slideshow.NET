...
namespace TSOP.SlideShow
{
	...
	public class SlideShowSetupForm : System.Windows.Forms.Form, ISlideShowForm
	{
        ...
        private ToolBarButton toolBarButtonSearch;
		...
		private void InitializeComponent()
		{
            ...
            this.toolBarButtonSearch = new ToolBarButton();
            ...
            this._listViewImages.DoubleClick += new System.EventHandler(this._listViewImages_DoubleClick);
            this._listViewImages.DragOver += new System.Windows.Forms.DragEventHandler(this._listViewImages_DragOver);
            ...
            this._listViewImages.ItemDrag += new System.Windows.Forms.ItemDragEventHandler(this._listViewImages_ItemDrag);
            ...
            this.toolBarButtonSearch.Text = StringConstants.GetString(StringNames.SEARCH);
            this.toolBarButtonSearch.ToolTipText = "Searches image items for the selected keywords";
            // 
            ...
            // _btnTest
            // 
            this._btnTest.Location = new System.Drawing.Point(658, 49);
            this._btnTest.Name = "_btnTest";
            this._btnTest.Size = new System.Drawing.Size(75, 23);
            this._btnTest.TabIndex = 34;
		...
		#region Members
            ...
            private ImageItemList _searchResults;
		...
		public SlideShowSetupForm(bool loadFromRegistry)
		{
			...
			_allKeywords = new List<string>(23);
		...
		{
            if (_searchMode)
                return _searchResults.GetImageItem(imageID.Index);
            else
		...
		private void Promote(bool promote)
		{
			...
			{   // todo: there is a bug here.  when selecting an image item by using up/down arrows,
                ...
                if (promote)
                {
                    // todo: for some reason, when promoting a single image item, all recently
                    // unselected image items are also getting promoted
                    WriteDiagnostics(ii, (ImageID)lvi.Tag, _listViewImages.SelectedItems.IndexOf(lvi), "!!! BEFORE !!!");
                    // todo: remove this after debugging because it could cause an out of range exception
                    ImageID iidNext = (ImageID) _listViewImages.Items[lvi.Index + 1].Tag;
                    ImageItem iiNext = GetImageItem(iidNext);
                    WriteDiagnostics(iiNext, iidNext, -1, "!!! BEFORE !!!");
                    ii.Promote();
                    WriteDiagnostics(ii, (ImageID)lvi.Tag, _listViewImages.SelectedItems.IndexOf(lvi), "!!! AFTER !!!");
                    WriteDiagnostics(iiNext, iidNext, -1, "!!! AFTER !!!");
                }
                else
                {
                    WriteDiagnostics(ii, (ImageID)lvi.Tag, _listViewImages.SelectedItems.IndexOf(lvi), "!!! BEFORE !!!");
		...
		private void _menuItemFile_Save_Click(object sender, System.EventArgs e)
		{
            ...
            if (_lastFileName == StringConstants.REGISTRY_FILENAME || _lastFileName == "" || CurrentConfigFile.FileName == null || CurrentConfigFile.FileName == "")
            {
		...
		private void _mnuItemCtxtEditKW_Click(object sender, System.EventArgs e)
		{
			...
			KeywordEditForm kf = new KeywordEditForm(false);
			...
			kf.SelectedKeywords = Utilities.BuildDistinctKeywords(selectedImageItems);
			kf.ShowDialog();
		...
		private void _mnuItemCtxtRemoveKW_Click(object sender, System.EventArgs e)
		{
			...
			kf.Keywords = Utilities.BuildDistinctKeywords(selectedImageItems);
        ...
        private void _listViewImages_DoubleClick(object sender, EventArgs e)
        {
            if (_preview == null)
            ...
            {
                _preview.Show();
                _toolBar.Buttons[(int)ToolBarButtons.Preview].Pushed = true;
            }
            
        }

        private void _listViewImages_ItemDrag(object sender, ItemDragEventArgs e)
        {
            string[] fileArray = new string[_listViewImages.SelectedItems.Count];

            for (int i = 0; i < _listViewImages.SelectedItems.Count; i++)
            {
                fileArray[i] = System.IO.Path.Combine(_listViewImages.SelectedItems[i].SubItems[(int)ListViewColumns.FilePath].Text,
                                    _listViewImages.SelectedItems[i].SubItems[(int)ListViewColumns.FileName].Text);
            }

            _listViewImages.DoDragDrop(new DataObject(DataFormats.FileDrop, fileArray), DragDropEffects.Copy);
        ...
        private void _listViewImages_DragDrop(object sender, DragEventArgs e)
        {
            ...
            // todo: when a drag drop fails, the source window may be covering this application, and then you don't see
            // the error message dialog.  It should be that after dropping, this application becomes the topmost window.

            AddFiles(fileNames);
        }

        private void _listViewImages_DragOver(object sender, DragEventArgs e)
        {
...
//            if (((ContextMenu)((MenuItem)sender).Parent).SourceControl.Name == "PreviewForm")
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                else
                {
                    foreach (string errorFile in errorFiles)
                        errorMsgs.Add(String.Format("Failed to {0} {1}", verb, errorFile));
        ...
        private void DeleteImageFiles()
        {
            ...
            {
                foreach (string errorFile in errorFiles)
                    errorMsgs.Add(String.Format("Failed to Delete {0}", errorFile));
        ...
        {
            RemoveFiles(new List<string>(0));
        }

        /// <summary>
        /// Removes files from the list view
        /// </summary>
        /// <param name="excludeFiles">array of string describing files that failed to be moved or deleted</param>
		private void RemoveFiles(List<string> excludeFiles)
		{
            ...
            for (int i = 0; i < _listViewImages.SelectedItems.Count; i++)
            {
                // if an item was selected, but is in the failure list, don't remove it
                ListViewItem lvi = _listViewImages.SelectedItems[i];
                string selectedPath = System.IO.Path.Combine(lvi.SubItems[(int)ListViewColumns.FilePath].Text, lvi.SubItems[(int)ListViewColumns.FileName].Text);
                if (!excludeFiles.Contains(selectedPath))
                    Tags.Add((ImageID)(lvi.Tag));
            }
            Tags.Sort();
        ...
        private void AddFilesBySearch(bool recursive)
        {
            ...
            if (dlg.ShowDialog(this) == DialogResult.OK)
            {
                ...
                allFiles.AddRange(System.IO.Directory.GetFiles(dlg.SelectedPath, "*.JPEG", opt));
                ...
                for (int i = 0; i < _listViewScripts.SelectedItems.Count; i++)
                {
                   ...
                   foreach (string fileName in allFiles)
                   {
                        ...
                        if (LoadThumbnailData)
                            ii.ReadThumbnailDataFromFile();
                        if (sf.ImageItemCollection.AddImageItem(ii) == -1)
        ...
        /// <summary>
        /// Adds Files to the selected script[s].  If fileNames array is null, a dialog box is used to select files
        /// </summary>
        /// <param name="fileNames">a string array of file pathnames</param>
		private void AddFiles(string[] fileNames)
		{
            ...
            if (fileNames == null)
            {
 			...
 			if (dlgResult == DialogResult.OK)
			{
				if (dlg != null && RememberFolders)
					_regSettings.SetOpenDir(System.IO.Path.GetDirectoryName(dlg.FileName));
                ArrayList notAdded = new ArrayList();
                float scriptProgressInterval = 100 / _listViewScripts.SelectedIndices.Count;
                float fileProgressInterval = scriptProgressInterval / fileNames.GetLength(0);
 				for (int i = 0; i < _listViewScripts.SelectedItems.Count; i++)
				{
					String scriptName = CurrentConfigFile.ScriptFileList[_listViewScripts.SelectedIndices[i]].PathAndFileName;
					ScriptFile sf = CurrentConfigFile.ScriptFileList.GetScriptByName(scriptName);
					...
					{
                       ImageItem ii = new ImageItem(fileNames[j]);
						if (LoadThumbnailData)
							ii.ReadThumbnailDataFromFile();
						if (sf.ImageItemCollection.AddImageItem(ii) == -1)
                            notAdded.Add(ii.PathAndFileName);
		...
		private void PopulateImagesList()
		{
			...
			for (int i = 0; i < _listViewScripts.SelectedIndices.Count; i++)
			{
                ...
                BindImageItemList(sf.ImageItemCollection, scriptName, _listViewScripts.SelectedIndices[i]);
			}
        ...
        private void BindImageItemList(ImageItemList itemList, string scriptName, int colorIndex)
        {
            //foreach (ImageItem ii in sf.ImageItemCollection)
            ListViewItem[] lvItems = new ListViewItem[itemList.Count];
            for (int index = 0; index < itemList.Count; index++)
            {
                ImageItem ii = (ImageItem)itemList[index];
        ...
        // need to implement the AND / OR logic
        // the image id is getting screwed up when the _searchResults list is created
        private void SearchKeywords()
        {
            // todo: this will probably be replaced with a more sophisticated search dialog
            KeywordEditForm dlg = new KeywordEditForm(true);
            dlg.Keywords = _allKeywords;
            dlg.Text = "Search Keywords";

            if (dlg.ShowDialog() == DialogResult.OK)
            {
                // iterate through each selected ScriptFile, calling its search function to get a subset of the ImageItems
                _searchResults = _configFile.ScriptFileList[0].ImageItemCollection.SearchKeywords(dlg.SelectedKeywords);
                

                // add the ImageItem subset to a new ImageItemList

                // bind the temporary ImageItemList to the ListView
                // todo: if any items were selected, save the selections, then after re-populating, re-select any items that still exist
                // should probably add a parameter to disable this feature
                _listViewImages.Items.Clear();
                Cursor.Current = Cursors.WaitCursor;
                ...
                BindImageItemList(_searchResults, _configFile.ScriptFileList[0].PathAndFileName, 1);
                ...
                // change the search button to a "Cancel Search" button
                _searchMode = true;
                _toolBar.Buttons[(int) ToolBarButtons.Search].Text = StringConstants.GetString(StringNames.CANCEL_SEARCH);
            }
        ...
        private void CancelSearch()
        {
            PopulateImagesList();
            _searchMode = false;
            _toolBar.Buttons[(int) ToolBarButtons.Search].Text = StringConstants.GetString(StringNames.SEARCH);
            _searchResults = null;
        }
        ...
        private void _toolBar_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
			switch (_toolBar.Buttons.IndexOf(e.Button))
			{
                ...
                case (int) ToolBarButtons.Save:
                     ...
                     break;
                case (int) ToolBarButtons.Search:
                    if (_searchMode)
                        CancelSearch();
                    else
                        SearchKeywords();
        ...
        private void WriteSelectedLVIDiagnostics()
        {
            if (_listViewImages.SelectedItems.Count == 0)
                return;
            ...
            for (int i = 0; i < _listViewImages.SelectedIndices.Count; i++)
            {
                ...
                WriteDiagnostics(ii, iid, i, "");
            }

        }

        private void WriteDiagnostics(ImageItem ii, ImageID iid)
        {
            WriteDiagnostics(ii, iid, -1, "");
        }

        private void WriteDiagnostics(ImageItem ii, ImageID iid, int index, string addlText)
        {
            if (addlText != "")
                System.Diagnostics.Debug.WriteLine(addlText);
            System.Text.StringBuilder sBldr = new System.Text.StringBuilder("Selected[" + index + "]: ");
            sBldr.Append("_imageIndex = " + _imageIndex.ToString());
            sBldr.Append(": ImageID = " + iid.ScriptFile + " " + iid.Index);
            if (index >= 0)
                sBldr.Append(": LV Index = " + _listViewImages.SelectedIndices[index].ToString());
