...
namespace TSOP.SlideShow
{
	...
	/// </summary>
	[Serializable]
	public class ImageItem : ISerializable, IDisposable
	{
		#region Members
...
//			}
                ...
                if (_imageObj == null)
                    try
                    {
                        if (File.Exists(PathAndFileName))
                        {
                            ...
                            _imageObj = Image.FromStream(sr.BaseStream);
                            sr.Close();                                     // this method releases the file immediately
                            //_imageObj = Image.FromFile(PathAndFileName);  // this method keeps the file locked until the Image is Disposed().
                        }
                        else
		...
		protected ImageItem(SerializationInfo info, StreamingContext context)
		{
			...
			_keywordList = (List<string>) info.GetValue("KeywordList", typeof(List<string>));
// ??		_previewScaleFactor = (float) info.GetDouble("PreviewScaleFactor");
		...
		private void Init()
		{
			...
			this._keywordList = new List<string>();
        ...
        {
            if (_imageObj != null) _imageObj.Dispose();
            _imageObj = null;
           System.GC.Collect();
        }
